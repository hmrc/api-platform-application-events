...
object EventToDisplay {
  ...
  def display(evt: ApplicationEvent): DisplayEvent = {
    val (eventType, metaData) = evt match {
      ...
      case PpnsCallBackUriUpdatedEvent  (_, _, _, _, boxId, boxName, oldCallbackUrl, newCallbackUrl)        => ("Ppns CallBackUri Updated", List(s"boxName: ${boxName}", s"oldCallBackUrl: ${oldCallbackUrl}", s"newCallBackUrl: ${newCallbackUrl}"))
      case RedirectUrisUpdatedV2(_, _, _, _, oldRedirectUris: List[String], newRedirectUris: List[String])  => ("Redirect Uris Updated", List(s"""oldRedirectUris: ${oldRedirectUris.mkString(",")}""", s"""newRedirectUris: ${newRedirectUris.mkString(",")}"""))
      ...
      case ResponsibleIndividualChanged(_, _, _, _, previousResponsibleIndividualName, previousResponsibleIndividualEmail, newResponsibleIndividualName,newResponsibleIndividualEmail, submissionId, submissionIndex, _, requestingAdminName, requestingAdminEmail) =>
      ...
      case ResponsibleIndividualChangedToSelf(_, _, _, _, previousResponsibleIndividualName, previousResponsibleIndividualEmail, submissionId, submissionIndex, requestingAdminName, requestingAdminEmail) =>
      ...
      case ResponsibleIndividualDeclined(_, _, _, _, responsibleIndividualName, responsibleIndividualEmail, submissionId, submissionIndex, _, requestingAdminName, requestingAdminEmail) =>
      ...
      case ResponsibleIndividualDeclinedUpdate(_, _, _, _, responsibleIndividualName, responsibleIndividualEmail, submissionId, submissionIndex, _, requestingAdminName, requestingAdminEmail) =>
      ...
      case ResponsibleIndividualDidNotVerify(_, _, _, _, responsibleIndividualName, responsibleIndividualEmail, submissionId, submissionIndex, _, requestingAdminName, requestingAdminEmail) =>
      ...
      case ResponsibleIndividualDeclinedOrDidNotVerify(_, _, _, _,  responsibleIndividualName, responsibleIndividualEmail, submissionId, submissionIndex, code,  requestingAdminName,  requestingAdminEmail) => 
      ...
      case ResponsibleIndividualSet(_, _, _, _,   responsibleIndividualName,  responsibleIndividualEmail, submissionId, submissionIndex, code, requestingAdminName, requestingAdminEmail) => 
        ("Responsible Individual Set",          List(
            s"Submission Id: ${submissionId.value} - ${submissionIndex}",
            s"Requested by ${requestingAdminName} @ ${requestingAdminEmail.text}",
            s"To Reponsible Individual: ${responsibleIndividualName} @ ${responsibleIndividualEmail.text}"
          ))
      case ResponsibleIndividualVerificationStarted(_, _, _, _, applicationName, requestingAdminName, requestingAdminEmail, responsibleIndividualName, responsibleIndividualEmail, submissionId, submissionIndex, verificationId) => 
      ...
      case ApplicationApprovalRequestDeclined(_, _, _, _, decliningUserName, decliningUserEmail, submissionId, submissionIndex, reasons, requestingAdminName, requestingAdminEmail) => 
      ...
      case ProductionAppTermsConditionsLocationChanged(_, _, _, _, oldLocation, newLocation)  => ("T&Cs Changed", List(s"From: ${oldLocation.describe()}", s"To: ${newLocation.describe()}"))
    ...
    DisplayEvent(evt.id, evt.applicationId, evt.eventDateTime, evt.actor, EventTags.tag(evt), eventType, metaData)
  }
}
