...
abstract class EventSpec extends HmrcSpec with Matchers with Inside with BaseJsonFormattersSpec with EventTestData {
  ...
  def testDisplay(applicationEvent: ApplicationEvent, @unused expectedEventTag: EventTag, expectedEventType: String, expectedTextItems: List[String]) = {
    inside(applicationEvent.asMetaData()) {
      case (eventType, meta) =>
