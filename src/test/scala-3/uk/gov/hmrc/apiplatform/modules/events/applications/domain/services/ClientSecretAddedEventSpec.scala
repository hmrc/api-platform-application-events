...
class ClientSecretAddedEventSpec extends EventSpec {
  ...
  "ClientSecretAdded" should {
    ...
    val jsonText =
      raw"""{"id":"$anEventId","applicationId":"$anAppId","eventDateTime":"$instantText","actor":{"email":"${appCollaborator.email}","actorType":"COLLABORATOR"},"clientSecretId":"someClientId","eventType":"CLIENT_SECRET_ADDED"}"""
    ...
    "display ClientSecretRemoved correctly" in {
      testDisplay(clientSecretAdded, EventTag.ClientSecret, "Client Secret Added", List(s"Id: $aClientSecretId"))
    }
  }
