...
class ClientSecretRemovedV2EventSpec extends EventSpec {
  ...
  "ClientSecretRemovedV2" should {
    ...
    val jsonText =
      raw"""{"id":"$anEventId","applicationId":"$anAppId","eventDateTime":"$instantText","actor":{"email":"${appCollaborator.email}"},"clientSecretId":"someClientId","clientSecretName":"someClientSecretName","eventType":"CLIENT_SECRET_REMOVED_V2"}"""
    ...
    "display ClientSecretRemoved correctly" in {
      testDisplay(clientSecretRemovedV2, EventTag.ClientSecret, "Client Secret Removed", List("Name: someClientSecretName"))
    }
  }
