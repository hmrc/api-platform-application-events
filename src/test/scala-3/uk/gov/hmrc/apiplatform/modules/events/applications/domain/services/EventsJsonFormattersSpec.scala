...
class EventsJsonFormattersSpec extends BaseJsonFormattersSpec {
  ...
  "EventsInterServiceCallJsonFormatters" when {
    ...
    "given a ResponsibleIndividualChanged event" should {
      val submissionId = SubmissionId.random
      val now          = Instant.now
      val nowText      = dateTimeFormatter.format(now)
    ...
    "given a ResponsibleIndividualDeclinedOrDidNotVerify event" should {
      ...
      "write to json" in {
        Json.prettyPrint(Json.toJson(evt)) shouldBe Json.prettyPrint(Json.parse(jsonText))
      }
    }

    "dump some json" in {
      import LaxEmailAddress.StringSyntax.toLaxEmail
