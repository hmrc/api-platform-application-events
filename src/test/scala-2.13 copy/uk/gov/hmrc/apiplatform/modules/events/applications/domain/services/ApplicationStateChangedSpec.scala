...
class ApplicationStateChangedSpec extends EventSpec {
  ...
  "ApplicationStateChanged" should {
    import EventsInterServiceCallJsonFormatters.*
    ...
    val applicationStateChanged: ApplicationEvent = ApplicationStateChanged(
      ...
      State.PendingGatekeeperApproval.toString,
      requestingAdminName,
      requestingEmail
    )

    val jsonText =
      raw"""{"id":"$anEventId","applicationId":"$anAppId","eventDateTime":"$instantText","actor":{"email":"${appCollaborator.email}","actorType":"COLLABORATOR"},"oldAppState":"TESTING","newAppState":"PENDING_GATEKEEPER_APPROVAL","requestingAdminName":"$requestingAdminName","requestingAdminEmail":"${requestingEmail.text}","eventType":"APPLICATION_STATE_CHANGED"}"""
    ...
    "display ApplicationStateChanged correctly" in {
      testDisplay(
        ...
        List(State.Testing.toString, State.PendingGatekeeperApproval.toString, requestingAdminName, requestingEmail.text)
      )
    }
  }
